# anonymator/ui/spatial_canvas.py
...
def scene_rect_to_source(x0: float, y0: float, x1: float, y1: float,
                         zoom: float) -> Rect:
    """Convertit un rectangle en coordonnées scène (pixels affichés) vers
    l'unité de la source, en normalisant l'ordre des coins.

    `zoom` est le facteur de rendu : 2.0 pour une page PDF rendue à 2x (l'unité
    source est alors le point PDF), 1.0 pour une image (déjà en pixels)."""
    return (min(x0, x1) / zoom, min(y0, y1) / zoom,
            max(x0, x1) / zoom, max(y0, y1) / zoom)


class SpatialCanvas(QGraphicsView):
    """Affiche une source rendue (page PDF ou image) + overlays de rectangles.
    En mode tracé, un glisser produit un rectangle manuel, émis dans l'unité de
    la source et non en pixels affichés — le zoom d'affichage n'influe pas."""

    manual_rect_drawn = Signal(tuple)   # (x0, y0, x1, y1) dans l'unité source
    ...
    def _finish_manual(self, p0: tuple[float, float],
        ...
        pts = scene_rect_to_source(p0[0], p0[1], p1[0], p1[1], self._zoom)
