...
- ~~**Mode texte ignore les décochages du panneau**~~ **[RÉSOLU]** : `PdfScreen.run_text` route désormais via l'ensemble retenu de la session dès qu'une analyse a eu lieu — `pdf_io.anonymize_pdf_text_from_session` masque **page par page** (pas de rebasage d'offsets) uniquement les entités retenues (décochages type/valeur honorés), puis joint comme `anonymize_pdf_text`. Une valeur décochée reste en clair dans le `.txt`, cohérent avec le caviardage. Sans analyse préalable, `run_text` conserve le repli re-détection via `anonymize_pdf_text`. Couvert par `test_text_run_honors_review_decochage` (screen), `test_anonymize_pdf_text_from_session_honors_decochage` (pdf_io) et `test_retained_entities_by_page_*` (session).
